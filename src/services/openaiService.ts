...
export async function analyzeAreaPreferences(
  ...
  try {
    ...
    const prompt = mapMode === 'london' 
      ...
      : getUKPrompt(userInput);

    console.log(`Making request to OpenAI API with GPT-4o model in ${mapMode} mode`);
...
function getUKPrompt(userInput: string): string {
  return `
...
Only include real cities and towns within the United Kingdom, covering a range of sizes from small towns to major cities.
...
IMPORTANT: Limit your response to a maximum of 10 locations. Ensure all locations are real UK towns and cities with accurate information and actual UK coordinates.
Sort by match percentage in descending order. Make sure to include locations from different regions of the UK for diversity.
PROVIDE OUTPUT IN JSON FORMAT ONLY.
  `;
}
